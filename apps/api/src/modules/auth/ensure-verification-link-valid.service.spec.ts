...
import { EnsureVerificationLinkValidService } from './ensure-verification-link-valid.service'

function sha256(value: string) {
  return createHash('sha256').update(value).digest('hex')
}

describe('EnsureVerificationLinkValidService', () => {
  let usersRepository: InMemoryUsersRepository
  let emailVerificationTokensRepository: InMemoryEmailVerificationTokensRepository
  let mailProvider: FakeMailProvider
  let sut: EnsureVerificationLinkValidService

  beforeEach(() => {
    usersRepository = new InMemoryUsersRepository()
    emailVerificationTokensRepository = new InMemoryEmailVerificationTokensRepository()
    mailProvider = new FakeMailProvider()
    sut = new EnsureVerificationLinkValidService(
      usersRepository,
  ...
  it('should do nothing when user is already verified', async () => {
  ...
  it('should do nothing when a valid token already exists', async () => {
    const user = await usersRepository.create({
      name: 'John Doe',
      email: 'john@example.com',
      password_hash: await hash('123456', 6),
  ...
  it('should send a new email when no token exists', async () => {
    const user = await usersRepository.create({
      name: 'John Doe',
      email: 'john@example.com',
      password_hash: await hash('123456', 6),
  ...
  it('should invalidate expired token and send a new email', async () => {
    const user = await usersRepository.create({
      name: 'John Doe',
      email: 'john@example.com',
      password_hash: await hash('123456', 6),
  ...
  it('should send a new email when latest token is already used', async () => {
    const user = await usersRepository.create({
      name: 'John Doe',
      email: 'john@example.com',
      password_hash: await hash('123456', 6),
    ...
    const token = await emailVerificationTokensRepository.create({
      tokenHash: sha256('used-token'),
      userId: user.id,
      expiresAt: new Date(Date.now() + 1000 * 60 * 60 * 24),
    })
    await emailVerificationTokensRepository.markAsUsed(token.id)
